...
def organize_dat(numbers: list[float]) -> list[tuple[int, list[float]]]:
    c = len(cols)
    data = [
        numbers[k:k+3]
        for k in range(0, len(numbers) - c + 1, c)
    ]
    data2= zip(itertools.count(0, step=50), data)
    return list(data2)


def save_dat(fname: str, data: list[tuple[int, list[float]]]):
    fmt = "{t}, {ax:.4f}, {ay:.4f}, {az:.4f}"
    c = [fmt.format(t=dp[0], ax=dp[1][0], ay=dp[1][1], az=dp[1][2]) for dp in data]
    with open(fname, "w+", newline="") as csvfile:
        csvfile.writelines(c)
